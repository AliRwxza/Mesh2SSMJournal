...
def validate_inspect(epoch):
	...
	for data in val_iter:
		...
		x = data['true_pointcloud']
		idx = data['idx']
		break

	with torch.no_grad():
		correspondences_pred = model.predict(vertices = x.to(args.device), idx = idx.to(args.device))

	z = torch.randn([args.train_batch_size, args.emb_dims]).to(args.device)
